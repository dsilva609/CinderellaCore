...
namespace CinderellaCore.Services.Features.Statistics
{
    public class StatisticService : IStatisticService
    {
        ...
        public int GetNumAlbums(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _items.Count(x => x.GetType() == typeof(Model.Models.Album))
                : _items.Count(x => x.UserID == userID && x.GetType() == typeof(Model.Models.Album));

        public int GetNumBooks(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _items.Count(x => x.GetType() == typeof(Model.Models.Book))
                : _items.Count(x => x.UserID == userID && x.GetType() == typeof(Model.Models.Book));

        public int GetNumMoviesShows(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _items.Count(x => x.GetType() == typeof(Model.Models.Movie))
                : _items.Count(x => x.UserID == userID && x.GetType() == typeof(Model.Models.Movie));

        public int GetNumGames(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _items.Count(x => x.GetType() == typeof(Model.Models.Game))
                : _items.Count(x => x.UserID == userID && x.GetType() == typeof(Model.Models.Game));
